const NOCODB_API_URL = 'https://db.hpb.edu.vn';
const NOCODB_API_TOKEN = '8m1N0qDkakGPT_Xq4MXBaA2gw5hWrTAkZVKJ3Wd_';
const TABLE_ID = 'm9j1kt8ml62g5r5'; // TABLE_SYNC_LOGS

async function debugSyncLogs() {
    try {
        console.log(`Fetching sync logs...`);

        // Sort by CreatedAt descending
        const url = `${NOCODB_API_URL}/api/v2/tables/${TABLE_ID}/records?limit=5&sort=-CreatedAt`;

        const response = await fetch(url, {
            headers: {
                'xc-token': NOCODB_API_TOKEN
            }
        });

        if (!response.ok) {
            const text = await response.text();
            throw new Error(`API Error: ${response.status} ${response.statusText} - ${text}`);
        }

        const data = await response.json();
        console.log(`Found ${data.list.length} logs.`);

        if (data.list.length > 0) {
            data.list.forEach(record => {
                console.log('------------------------------------------------');
                console.log(`Started At: ${record.started_at}`);
                console.log(`Finished At: ${record.finished_at}`);
                console.log(`Status: ${record.status}`);
                console.log(`Records Processed: ${record.records_processed}`);
                console.log(`Error Message: ${record.error_message}`);
            });
        }

    } catch (error) {
        console.error('Error:', error);
    }
}

debugSyncLogs();
