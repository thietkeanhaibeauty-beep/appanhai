...
export const getInsightsByUserAndDate = async (
  ...
  try {
    // CRITICAL: Filter by both user_id AND account_id to prevent data mixing
    let whereClause: string;

    if (accountId) {
      // ✅ SAFE: Filter by both user_id and account_id
      whereClause = encodeURIComponent(`(user_id,eq,${userId})~and(account_id,eq,${accountId})`);

    } else {
      // ⚠️ UNSAFE: Only filtering by user_id (backward compatibility)
      whereClause = encodeURIComponent(`(user_id,eq,${userId})`);
      console.warn('⚠️ WARNING: Fetching insights WITHOUT account_id filter. This may cause data mixing for multi-account users!');
    }

    const url = `${getNocoDBUrl(NOCODB_CONFIG.TABLES.FACEBOOK_INSIGHTS)}?where=${whereClause}&offset=${offset}&limit=${limit}&sort=-date_start`;
    ...
    const filtered = (data.list || []).filter(insight => {
      if (!insight.date_start) return false;
      return insight.date_start >= startDate && insight.date_start <= endDate;
    });
    ...
    const fixedInsights = filtered.map(insight => {
      ...
      if (!insight.level) {
        ...
        console.warn('⚠️ Auto-fixed missing level for record:', {
          id: insight.Id,
          campaign_id: insight.campaign_id,
          adset_id: insight.adset_id,
          ad_id: insight.ad_id,
          inferred_level: insight.level
        });
      }
      return insight;
    });
      ...
      await Promise.all(
        batch.map(async (insight) => {
          try {
            ...
            const checkUrl = `${getNocoDBUrl(NOCODB_CONFIG.TABLES.FACEBOOK_INSIGHTS)}?where=${whereClause}&limit=1`;
            ...
            if (existingRecord && existingRecord.Id) {
              ...
              if (Object.keys(changedOnly).length === 0) {
                ...
                const updateUrl = getNocoDBUrl(NOCODB_CONFIG.TABLES.FACEBOOK_INSIGHTS);
                const payload = {
              ...
              // INSERT new record
              const insertUrl = getNocoDBUrl(NOCODB_CONFIG.TABLES.FACEBOOK_INSIGHTS);
...
export const deleteInsightsByDate = async (userId: string, beforeDate: string): Promise<void> => {
  try {
    ...
    const url = `${getNocoDBUrl(NOCODB_CONFIG.TABLES.FACEBOOK_INSIGHTS)}?where=${whereClause}&limit=10000`;
    ...
    await Promise.all(
      toDelete.map(async (record) => {
        if (record.Id) {
          ...
          const deleteUrl = `${getNocoDBUrl(NOCODB_CONFIG.TABLES.FACEBOOK_INSIGHTS, String(record.Id))}`;
