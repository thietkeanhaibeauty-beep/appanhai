  ...
  try {
    ...
    if (templateCommand.type === 'use' && templateCommand.templateName && userId) {
      ...
      if (loadedTemplate) {
        ...
        // ✅ NEW: Parse "1:" prefix for custom campaign name
        let customCampaignName: string | null = null;
        const campaignNameMatch = remainText.match(/^1[:：]\s*(.+?)(?:\n|$)/i);
        if (campaignNameMatch) {
          customCampaignName = campaignNameMatch[1].trim();
          // Remove the "1:..." line from content
          remainText = remainText.replace(/^1[:：]\s*.+?(?:\n|$)/i, '').trim();
          console.log(`✅ Custom campaign name detected: "${customCampaignName}"`);
        }
        ...
        console.log('🔍 Smart Merge Analysis:', {
          ...
          customCampaignName: customCampaignName || 'none',
        ...
        aiResponse = {
          campaignType: finalCampaignType,
          // ✅ Use custom name if provided, else fallback to template
          campaignName: customCampaignName || loadedTemplate.campaign_name || loadedTemplate.name?.replace(/@#/g, '') || 'Campaign',
