/**
 * Shared metrics calculator for notification system
 * Extracted from useInsightsMetrics hook for reuse in edge functions
 */
...
/**
 * Calculate all metrics from insights array
 * Pure function - no side effects
 */
export function calculateInsightsMetrics(insights: BaseInsight[]): InsightsMetrics {
...
/**
 * Filter metrics to only include selected keys
 */
export function filterSelectedMetrics(
  allMetrics: InsightsMetrics,
  selectedKeys: string[]
): Record<string, number> {
  const filtered: Record<string, number> = {};
  
  selectedKeys.forEach(key => {
    if (key in allMetrics) {
      filtered[key] = allMetrics[key as keyof InsightsMetrics];
    }
  });
