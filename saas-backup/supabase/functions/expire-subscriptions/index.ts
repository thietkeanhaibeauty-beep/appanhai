    ...
    for (const sub of expiredSubs) {
      try {
        const nocodbResponse = await fetch(
          `${Deno.env.get('NOCODB_BASE_URL') || 'https://db.hpb.edu.vn'}/api/v2/tables/${Deno.env.get('NOCODB_USER_SUBSCRIPTIONS_TABLE') || 'myjov622ntt3j73'}/records`,
          {
            method: 'GET',
            headers: {
              'xc-token': Deno.env.get('NOCODB_API_TOKEN') || '1wrsHNcz_FNeptaeMvP7jqrcVpm0GtD_8JScOLGo',
        ...
        if (nocodbResponse.ok) {
          ...
          if (nocodbRecord) {
            await fetch(
              `${Deno.env.get('NOCODB_BASE_URL') || 'https://db.hpb.edu.vn'}/api/v2/tables/${Deno.env.get('NOCODB_USER_SUBSCRIPTIONS_TABLE') || 'myjov622ntt3j73'}/records/${nocodbRecord.Id}`,
              {
                method: 'PATCH',
                headers: {
                  'xc-token': Deno.env.get('NOCODB_API_TOKEN') || '1wrsHNcz_FNeptaeMvP7jqrcVpm0GtD_8JScOLGo',
